    ...
    <button
      ...
      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium transition-all border ${
        checked
          ...
          : "bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:border-indigo-300 dark:hover:border-indigo-600 hover:text-indigo-600 dark:hover:text-indigo-400"
      }`}
    >
...
function ExerciseRow({
  ...
  // Sync whenever the DB-backed initial state changes (date switch, query load, real-time update)
  ...
  return (
    <div
      className={`bg-white dark:bg-gray-900 border rounded-2xl p-4 shadow-sm transition-all ${
        allDone
          ? "border-emerald-300 dark:border-emerald-700 ring-1 ring-emerald-200 dark:ring-emerald-800 bg-emerald-50/30 dark:bg-emerald-950/30"
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        ...
        {allDone && (
          <div className="flex items-center gap-1.5 bg-emerald-100 dark:bg-emerald-900 text-emerald-700 dark:text-emerald-300 text-xs font-semibold px-2.5 py-1 rounded-xl flex-shrink-0">
      ...
      <main className="max-w-3xl mx-auto px-4 py-8 pb-24 sm:pb-8 page-enter">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-gray-100 transition-colors shadow-sm"
            >
                ...
                <button
                  ...
                  className={`flex-1 flex flex-col items-center gap-0.5 py-2 rounded-xl text-[10px] font-semibold transition-all ${
                    isSelected
                      ...
                      : hasWorkout
                      ? "bg-indigo-100 dark:bg-indigo-950 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-200 dark:hover:bg-indigo-900"
        ...
        {selectedGroups.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <div className="p-6 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl mb-5 shadow-sm">
              <Moon className="w-10 h-10 text-indigo-300 dark:text-indigo-600 mx-auto" />
            </div>
            <h2 className="text-xl font-semibold text-gray-700 dark:text-gray-300 mb-2">Rest day</h2>
            <p className="text-gray-400 dark:text-gray-500 text-sm max-w-xs">
              No workouts scheduled for {DAY_NAMES[selectedDay]}.
              {nextDay && (
                <> Next session is <span className="text-gray-700 dark:text-gray-300 font-medium">{nextDay.dayName}</span>.</>
              )}
            </p>
          </div>
        ) : (
            ...
            {selectedGroups.map((group) => (
              <div key={group._id}>
                ...
                {group.exercises.length === 0 ? (
                  <div className="text-center py-6 text-sm text-gray-400 dark:text-gray-500 bg-white dark:bg-gray-900 border border-dashed border-gray-200 dark:border-gray-700 rounded-2xl">
                    No exercises in this group yet.
                  </div>
                ) : sessions === undefined ? (
                  <div className="space-y-3">
                    {group.exercises.map((ge) => (
                      <div key={ge._id} className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-4 shadow-sm animate-pulse">
