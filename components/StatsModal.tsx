...
export default function StatsModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="modal-panel relative bg-white border border-gray-100 rounded-t-2xl sm:rounded-2xl p-5 sm:p-6 w-full sm:max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
        <button
          onClick={onClose}
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Your strength stats</p>
          </div>
        </div>
        ...
        <div className="bg-gray-50 border border-gray-100 rounded-2xl p-5 mb-4">
          <div className="flex items-end justify-between mb-3">
            <div>
              <p className="text-gray-400 text-xs mb-1">Your weight</p>
              <p className="text-3xl font-bold text-gray-900">
                {resistance} <span className="text-lg text-gray-400 font-normal">{resistanceUnit}</span>
              </p>
            </div>
          ...
          <div className="mt-3 flex items-center gap-3 flex-wrap">
            ...
            <span className="text-xs text-gray-500">{bwRatio}× bodyweight</span>
            {heightNote && (
              <>
                <span className="text-gray-300">·</span>
                <span className="text-xs text-gray-400 italic">{heightNote}</span>
              </>
            )}
          </div>
        </div>
            ...
            {dist.map(({ pct, val }) => {
              ...
              return (
                <div key={pct} className="flex items-center gap-3">
                  <span className="text-[11px] text-gray-400 w-14 flex-shrink-0">{pct}th %ile</span>
                  <div className="flex-1 bg-gray-100 rounded-full h-2 overflow-hidden">
                    <div
                      className={`h-full rounded-full ${isUser ? "bg-emerald-500" : "bg-gray-300"}`}
                      style={{ width: `${(val / maxVal) * 100}%` }}
                    />
                  </div>
                  <span className={`text-[11px] w-16 text-right flex-shrink-0 ${isUser ? "text-emerald-600 font-semibold" : "text-gray-400"}`}>
                    {val} lbs
        ...
        {/* User context */}
        <div className="bg-gray-50 border border-gray-100 rounded-xl p-3 text-xs text-gray-400 flex gap-4">
