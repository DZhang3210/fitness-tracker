...
export default function RoutineBrowser({ onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="modal-panel relative bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-2xl shadow-xl max-h-[90vh] flex flex-col">
        {/* Header */}
        <div className="p-5 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-semibold text-gray-900 dark:text-gray-100">Premade Routines</h2>
              <p className="text-xs text-gray-500 dark:text-gray-400">Pick a plan and we&apos;ll set everything up</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Filters */}
        <div className="px-5 py-3 border-b border-gray-100 dark:border-gray-800 flex gap-2 flex-wrap flex-shrink-0">
          ...
          {filtered.map((routine) => {
            ...
            return (
              ...
              >
                <div className="p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <h3 className="font-semibold text-gray-900 dark:text-gray-100 text-sm">{routine.name}</h3>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 leading-relaxed">{routine.description}</p>
                    ...
                    <div className="flex flex-col gap-2 flex-shrink-0">
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 rounded-xl text-[11px] text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors flex items-center gap-1"
                      >
                        Preview
                  ...
                  <div className="flex gap-1.5 mt-3 flex-wrap">
                    {routine.tags.map((tag) => (
                      <span key={tag} className="text-[10px] text-gray-400 dark:text-gray-500 border border-gray-200 dark:border-gray-700 rounded px-1.5 py-0.5 bg-gray-50 dark:bg-gray-800">
                        {tag}
                      </span>
                    ))}
                  </div>
                ...
                {isExpanded && (
                  <div className="border-t border-gray-100 dark:border-gray-700 px-4 pb-4 pt-3 space-y-3 bg-gray-50 dark:bg-gray-800/50">
                    {routine.groups.map((group) => (
                      <div key={group.title}>
                        <p className="text-[11px] font-semibold text-gray-600 dark:text-gray-400 mb-1.5">{group.title}</p>
                        <div className="grid grid-cols-2 gap-1">
                          {group.exerciseIds.map((id) => {
                            const ex = getExercise(id);
                            return (
                              <div key={id} className="text-[11px] text-gray-500 dark:text-gray-400 flex items-center gap-1.5">
